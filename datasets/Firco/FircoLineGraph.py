...
class FircoLineGraph(DGLDataset):
    # 77149 nodes, 90319 edges, 512 dims -> 90319 nodes, 90327 edges (as directed graph) or 90319 nodes, 158604737 edges
    # (as undirected graph)
    def __init__(self, subset=None, no_node_features=False, homo=True, aggregate=False, bidirected=True, initialized_instance=None):
        ...
        if initialized_instance is None:
            super().__init__(name="FircoLineGraph")
        else:
    ...
    def process(self):
        df_train = pd.read_csv(os.path.join(FircoEdgeToNodeDataFolder, "train_edges.csv"))
        df_val = pd.read_csv(os.path.join(FircoEdgeToNodeDataFolder, "val_edges.csv"))
        df_test = pd.read_csv(os.path.join(FircoEdgeToNodeDataFolder, "test_edges.csv"))
        train_embeds, train_labels = CM.IO.ImportFromPkl(os.path.join(FircoEdgeToNodeDataFolder, "train_embeds_labels.pkl"))
        val_embeds, val_labels = CM.IO.ImportFromPkl(os.path.join(FircoEdgeToNodeDataFolder, "val_embeds_labels.pkl"))
        test_embeds, test_labels = CM.IO.ImportFromPkl(os.path.join(FircoEdgeToNodeDataFolder, "test_embeds_labels.pkl"))
        ...
        # self.graph = dgl.line_graph(self.graph, backtracking=self.bidirected, shared=True)
...
if __name__ == "__main__":
    dataset = FircoLineGraph(subset="train")
    graph = dataset[0]
